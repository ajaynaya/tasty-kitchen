...
class RestaurantDetails extends Component {
    ...
    return (
      <>
        ...
        <ul className="food-items-container">
          {foodItems.map(each => (
            <Foods key={each.id} itemDetails={each} />
          ))}
        </ul>
      </>
    )
  ...
  renderRestaurantsDetailsBasedOnApiStatus = () => {
    const {apiStatus} = this.state
    switch (apiStatus) {
      case apiStatusConstants.progress:
        return this.renderLoader()
      case apiStatusConstants.success:
        return this.renderRestaurantsDetails()
      default:
        return null
    }
  }
  ...
  render() {
    return (
      <div className="rest-info-container">
        <Header />
        {this.renderRestaurantsDetailsBasedOnApiStatus()}
        <Footer />
      </div>
    )
  }
